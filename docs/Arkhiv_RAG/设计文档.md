...
- **状态**：持续更新（跟随当前代码库）
- **读者**：扩展 Arkhiv-RAG 的工程师（核心、领域插件、下游 Agent）
- **范围**：`Arkhiv_rag` 后端的架构及其对外接口（REST / SSE / MCP）。前端是 Core 能力的展示层，仅在约束后端时提及。

---

## 1. 上下文与范围

Arkhiv-RAG 是一个**行业无关、多租户（`kb_name`）的多模态 RAG 数据层**，面向 Agent 与 LLM。它只承担四项职责（ADR-008）：
...
- **双入库管线**，按「格式 + 内容形态」路由：Knowhere（语义骨架：类型化 chunk、`doc_nav` 章节树、知识图谱边）处理文本类文档；PixelRAG（视觉切片，2048 维）处理扫描件/图片文档。见 `Arkhiv_rag/ingest/router.py`。
- **多模态融合**，通过 `Arkhiv_visual` 上的四个锚点字段（`chunk_type`、`parent_section`、`content_summary`、`source_chunk_id`）锚定到 Knowhere 语义树，支持父文档召回（先 `type="section_summary"`，再按 `path` 前缀下钻）。
...
- **不做运行时领域切换**——每个实例一个 `default_namespace`（ADR-002）。多行业 = 多套部署（`Arkhiv_RAG_PROFILE`），而非运行时切换。
...
| 向量库     | Milvus 2.6（`pymilvus` + `llama-index-vector-stores-milvus`）                                                                                 | 每个 `plugin_namespace` 一个 Database；双集合 `Arkhiv_text`（1536 维）/ `Arkhiv_visual`（2048 维）；HNSW/DiskANN + 标量过滤。                                                                                                                                   |
...
| 前端       | Next.js 16 / React 19 / TS / pnpm / Tailwind v4 / HeroUI v3 / next-intl                                                                       | Core 展示层；本仓库垂类无 UI。                                                                                                                                                                                                                                  |
...
| 环节                  | 模型                                                                              | 运行方式                                                                                                         | 配置位置                                                                                                                        |
| :-------------------- | :-------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------ |
| **切片/解析（文本）** | Knowhere 语义解析（切块本身无 LLM）<br>章节导航/总结用 DeepSeek-V4-Pro + Qwen VLM | **parser 模式**：进程内 `knowhere-parse-sdk`，LLM 由 Eagle-RAG 映射<br>**api 模式**：Knowhere :5005 服务内部处理 | `settings.knowhere.parser`<br>← 全局 `settings.llm` / `settings.vlm`<br>（`_build_parser_config()`，`knowhere_adapter.py:199`） |
| **切片/解析（视觉）** | PixelRAG 渲染切片（无 LLM）                                                       | 进程内库 `pixelrag_render`                                                                                       | `settings.pixelrag`                                                                                                             |
| **向量化-文本**       | Qwen text-embedding-v4（1536 维，Embedding 模型，非 LLM）                         | 百炼 DashScope 云端 API                                                                                          | `settings.embedding.text`<br>（`DASHSCOPE_API_KEY`）                                                                            |
| **向量化-视觉**       | Qwen3-VL-Embedding-2B（2048 维）                                                  | **默认**：本地服务端（torch, cuda/mps/cpu）<br>**可切**：百炼 API                                                | `settings.embedding.visual`<br>（`VISUAL_EMBEDDING_PROVIDER`）                                                                  |
| **检索路由**          | DeepSeek-V4-Pro                                                                   | 百炼云端 API                                                                                                     | `settings.llm`（`LLMIntentSelector`，可设 `ROUTER_LLM_ENABLED=false` 关闭）                                                     |
| **ANN 检索**          | 无 LLM                                                                            | Milvus 2.6 上执行                                                                                                | —                                                                                                                               |
| **重排-文本**         | Qwen qwen3-rerank                                                                 | 百炼云端 API                                                                                                     | `settings.rerank.text`                                                                                                          |
| **重排-图像**         | Qwen3-VL-Embedding（复用视觉 Encoder 打分）                                       | 本地服务端                                                                                                       | `settings.rerank.visual`                                                                                                        |
| **生成**              | Qwen VLM（默认 `qwen3.6-flash`，文档口径 `Qwen-VL-Max`）                          | 百炼云端 API（原生 `dashscope.MultiModalConversation` 直连）                                                     | `settings.vlm`（`VLM_API_KEY`）                                                                                                 |
       ...
       └─ app.send_task(ingest_router)            # router_queue
            └─ ingest.router.ingest_router
                 ...
                 └─ pixelrag → pixelrag_build      # pixelrag_queue（并发 1）
                      └─ 渲染切片 → Qwen3-VL 2048 维 → upsert Arkhiv_visual
```

文本管线用 Qwen `text-embedding-v4`（1536 维）写入 `Arkhiv_text`；视觉切片带四个融合锚点字段落入 `Arkhiv_visual`。热路径 `PARSE` / `CHUNK` Hook 只增强节点，必须保留 Knowhere 的 `doc_nav` / `path` / 类型化 chunk（ADR-005）。

### 3.4 业务流：查询

```
POST /query | /search | /query/stream | /search/stream
  └─ ArkhivRouterQueryEngine
       ├─ 解析附件（惰性解析；不写 Milvus）
       ...
       └─ 生成（ArkhivMultimodalQueryEngine）
...
该图展示 Arkhiv-RAG 与外部世界的边界：Agent 不直接访问存储，Arkhiv-RAG 也不拥有 Agent 工作流——它只把检索与上下文组装暴露为工具。
...
- `Arkhiv_text`——1536 维文本块（HNSW/DiskANN），标量过滤 `kb_name`、`document_id`、`year`、`source_type`、`type`、`path`；去重主键 `(sha256, kb_name, plugin_namespace)`。
- `Arkhiv_visual`——2048 维视觉切片，带融合锚点：`chunk_type`（`tile|image|table`）、`parent_section`、`content_summary`、`source_chunk_id`。
...
遗留说明：`Arkhiv_rag/__init__.py` 的 docstring 仍保留重构前的描述 "for the finance and tax domain"；产品现已行业无关（AGENTS.md），建议在后续清理中更新。
